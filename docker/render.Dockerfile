...
ARG PG_PASSWORD
ARG PG_DATABASE
ARG STACKS_BLOCKCHAIN_API_PORT
ENV PG_HOST=${PG_HOST}
ENV PG_PORT=${PG_PORT}
ENV PG_USER=${PG_USER}
...
###################################
##  nginx config
RUN printf 'upstream stacks-blockchain-api {\n  server  localhost:%s;\n}\nserver {\n  listen       80;\n  server_name  localhost;\n  location /status {\n    default_type text/plain;\n    return 200 ok;\n  }\n  location / {\n    add_header Access-Control-Allow-Origin *;\n    proxy_pass http://stacks-blockchain-api/;\n    break;\n  }\n}' ${STACKS_BLOCKCHAIN_API_PORT} > /etc/nginx/conf.d/default.conf
...
RUN printf '#!/bin/sh\nexec 2>&1\ncase $STACKS_NETWORK in\n    testnet)\n        exec %s/stacks-node start --config=%s/Stacks-testnet.toml 2>&1\n        ;;\n    mocknet)\n        exec %s/stacks-node start --config=%s/Stacks-mocknet.toml 2>&1\n        ;;\n    *)\n        exec %s/stacks-node start --config=%s/Stacks-mainnet.toml 2>&1\n        ;;\nesac' ${STACKS_BLOCKCHAIN_DIR} ${STACKS_BLOCKCHAIN_DIR} ${STACKS_BLOCKCHAIN_DIR} ${STACKS_BLOCKCHAIN_DIR} ${STACKS_BLOCKCHAIN_DIR} ${STACKS_BLOCKCHAIN_DIR} > ${STACKS_SVC_DIR}/stacks-blockchain/run \
  ...
  && printf '#!/bin/sh\nexec /usr/sbin/nginx -g "daemon off;"' > ${STACKS_SVC_DIR}/nginx/run \
